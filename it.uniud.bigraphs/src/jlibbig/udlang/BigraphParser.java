...
public class BigraphParser extends Parser {
	...
	private class ParsedBigraph {
		 ...
		 * @param li
		 *            the list of outer and inner names
		 * @return the resulting bigraph
		 */
		private ParsedBigraph makeIon(String c, List<NameId> li) {
			polymorphicSites = true; // this bigraph can change the number of
										// its sites from 1 to 0.
			if (bb.getSignature().getByName(c) == null)
				throw new IllegalArgumentException("Control \"" + c
			...
			for (NameId name : li) {
				if (name == null) {
					// case: "-" , skip this port (already has an edge attached
					// to it)
					portIt.next();
					continue;
				}
				if (name.outer == true) {
					// case: "+p" or "p" , this name must appear in the
					// outerface of this bigraph
					OuterName outer = outerNames.get(name.name);
					if (outer == null)
						outerNames.put(name.name,
								outer = bb.addOuterName(name.name));
		 ...
		 * 
		 * @param x
		 *            the outer name that will be added. If null, an edge will
		 *            be generated.
		 * @param li
		 *            list of innernames that will be linked with x (the first
		 *            parameter, edge or outername)
		 * @return the resulting bigraph
		 */
		private ParsedBigraph makeLinks(NameId x, List<NameId> li) {
			...
			Set<String> innerNames = new HashSet<>(); // set of innernames
														// already added
			for (NameId name : li) {
				if (name != null) {
					if (innerNames.contains(name.name))
		...
		private ParsedBigraph makeSite(int n) {
			if (siteNames.contains(n))
				throw new IllegalArgumentException("The same site ($" + n
						+ ") can't appear multiple time in a single bigraph.");
		...
		/**
		 * Modify the link graph, according to the parameters parsed from a
		 * <x/xs> operator. Inner names in the list (second parameter) will
		 * appear in the resulting bigraph's innerface. Other names in the list
		 * will be linked with the outerface of the current bigraph, if they
		 * belongs to its outerface, otherwise they will appear in the resulting
		 * bigraph's innerface. Outernames of the current bigraph not linked
		 * with names in the list will appear in the resulting bigraph's
		 * outerface.
		 * 
		 * @param x
		 *            outername , it will appear in the resulting bigraph's
		 *            outerface.
		 * @param li
		 *            list of inner and outer names.
		 */
		private void addLinks(NameId x, List<NameId> li) {
			...
			for (NameId name : li) {
				if (name != null) {
					...
					if (name.outer) {
						if (bb_outer.contains(name.name)) { // name.name will be
															// an outername
							if (outer_done.contains(name.name))
								throw new RuntimeException(
										"Found the same OuterName ( "
												+ name.name
												+ " ) multiple time in the second argument of <x/xs> operation.");
							if (o == null)
					...
					if (!name.outer || asInner) { // name.name will be an
													// innername in the
													// resulting bigraph
			...
			// bigraph's outernames that don't appear in xs will appear in the
			// resulting bigraph's outerface
		 ...
		 * 
		 * @param p
		 *            the bigraph that will be composed with the current one.
		 */
		private void compose(ParsedBigraph p) {
			...
			Set<? extends InnerName> down_inner = new HashSet<>(
					this.bb.getInnerNames());
			Set<? extends OuterName> down_outer = new HashSet<>(
					this.bb.getOuterNames());
			Set<? extends InnerName> up_inner = new HashSet<>(
					p.bb.getInnerNames());
			Set<? extends OuterName> up_outer = new HashSet<>(
					p.bb.getOuterNames());

			// at the end of this cycle, all names in the inner bigraph are
			// handled
			for (OuterName in : down_outer) {
				...
				if (!exists) {
					...
					if (o != null) {
						...
						p.bb.addInnerName(in.getName(),
								p.bb.addOuterName(in.getName()));
					}
				}

				middle.addInnerName(in.getName(),
						middle.addOuterName(middleOuter));
			}

			// handle outer bigraph's inner names that aren't in inner bigraph's
			// outerface
					...
					for (InnerName ni : down_inner) {
						if (in.getName().equals(ni.getName()))
							throw new RuntimeException(
									"Innername ( -"
											+ ni.getName()
											+ " , contained in the first argument of a composition ) can't appear in the inner face of the resulting bigraph and can't be matched with an OuterName of the second argument.");

					}
					middle.addInnerName(in.getName(),
							middle.addOuterName(in.getName()));
					this.bb.addInnerName(in.getName(),
							this.bb.addOuterName(in.getName()));
				}
			}
		 ...
		 * 
		 * @param p
		 *            the bigraph that will be juxtaposed with the current one.
		 */
		private void juxtapose(ParsedBigraph p) {
			...
			// left_middle: bigraph used for renaming the outernames of the left
			// bigraph, it prevents name clashing
			...
			for (OuterName on : this.bb.getOuterNames()) {
				if (right_outer.contains(on.getName())) {
					...
					left_middle.addInnerName(on.getName(),
							left_middle.addOuterName(on.getName()));
					outer.addInnerName(on.getName(),
							outer.addOuterName(on.getName()));
				}
			}
	...
	public BigraphParser() {
		...
		actions = new Action[] { RETURN2, // [0] start = definitions reactions;
											// returns 'reactions' although none
											// is marked
				...
				new Action() { // [2] definitions = controls.sb
					public Symbol reduce(Symbol[] _symbols, int offset) {
						...
						_sys = new BigraphSystem(sb.makeSignature());
						return new Symbol(null);
					}
				}, new Action() { // [3] controls = controls.sb MODE.b VARID.v
									// COLON num.n SEMICOLON
					public Symbol reduce(Symbol[] _symbols, int offset) {
						final Symbol _symbol_sb = _symbols[offset + 1];
						final SignatureBuilder sb = (SignatureBuilder) _symbol_sb.value;
				...
				}, new Action() { // [4] controls = MODE.b VARID.v COLON num.n
									// SEMICOLON
					public Symbol reduce(Symbol[] _symbols, int offset) {
				...
				}, new Action() { // [5] reactions = big.r REACT big.s SEMICOLON
									// reactions
					public Symbol reduce(Symbol[] _symbols, int offset) {
				...
				new Action() { // [8] bigraphs = big.b SEMICOLON bigraphs
				...
				}, new Action() { // [12] big = TAGOPEN voidorname.x SLASH
									// names.l TAGCLOSE
					public Symbol reduce(Symbol[] _symbols, int offset) {
						...
						ParsedBigraph b;
						try {
							b = (new ParsedBigraph(_sys.getSignature()))
									.makeLinks(x, l);
						} catch (IllegalArgumentException e) {
							throw new IllegalArgumentException("Line: "
							...
							throw new RuntimeException("Line: "
									+ Symbol.getLine(_symbol_l.getStart())
									+ " - " + e.getMessage());
						}
					...
					public Symbol reduce(Symbol[] _symbols, int offset) {
						...
						final int n = (int) _symbol_n.value;
						return new Symbol((new ParsedBigraph(
								_sys.getSignature())).makeSite(n));
					}
					...
					public Symbol reduce(Symbol[] _symbols, int offset) {
						...
						try {
							b = (new ParsedBigraph(_sys.getSignature()))
									.makeIon(c, new LinkedList<NameId>());
						} catch (IllegalArgumentException e) {
							throw new IllegalArgumentException("Line: "
									+ Symbol.getLine(_symbol_c.getStart())
									+ " - " + e.getMessage());
