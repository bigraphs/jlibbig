...
public class PlaceGraph implements PlaceGraphAbst {
	...
	private final PlaceGraphFace _outer = new PGFace((List<PlaceGraphFacet>) (List<? extends PlaceGraphFacet>)_roots);
	@SuppressWarnings("unchecked")
	private final PlaceGraphFace _inner = new PGFace((List<PlaceGraphFacet>) (List<? extends PlaceGraphFacet>)_sites);
	
	// there is a lot of redundancy, but helps the generation of CSPs for the matching algorithm
	// parent map
	...
	//private Set<Parent> _ncst_filter = new HashSet<>(); // used to filter out inconsistencies introduced by composition
	// descendants (prnt-1+)
	private Map<Parent,Set<Child>> _dscn = null; //new HashMap<>();
	//private Set<Child> _dscn_filter = new HashSet<>(); // used to filter out inconsistencies introduced by composition
	...
	public PlaceGraph compose(PlaceGraph g){
		// does not perform a deep copy of plc
		if(this._sig != g._sig){
			throw new IllegalArgumentException("Incompatible signatures");
		}
		...
		// descendants and ancestors are lazy but viral, update these structures before making any change to this graph
		/*if(this._dscn != null && pg._dscn == null){
			pg.buildPrntClosure();
		...
		// add plc's nodes
		this._nodes.addAll(g._nodes);
		// bulk copy the parent of pg, this introduces some inconsistencies, but these will be addressed below
		this._prnt.putAll(g._prnt);
		for(PlaceGraphNode n: g._nodes){
			this._chld.put(n, new HashSet<Child>(g._chld.get(n)));
		}
		// iterate over roots and sites to be composed and glue parent maps accordingly
	...
	public static PlaceGraph makeIon(Signature<PlaceGraphControl> sig, PlaceGraphNode n){
	...
	public static PlaceGraph makeIon(Signature<PlaceGraphControl> sig, PlaceGraphControl control){
		return makeIon(sig,new PGNode(control)); 
	}
	
	public static PlaceGraph makeIon(Signature<PlaceGraphControl> sig, PlaceGraphControl control, String name){
		return makeIon(sig,new PGNode(control, name)); 
	}
	
	public static PlaceGraph makeId(Signature<PlaceGraphControl> sig, PlaceGraphFace f){
	...
	protected static class PGNode extends Named implements PlaceGraphNode{
		private final GraphControl _ctrl;
