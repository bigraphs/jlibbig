	...
	private static class MatchIterable implements Iterable<Match<Bigraph>> {
		...
		private void populateMatchQueue(boolean first) {
			...
			do {
				...
				/*
				 * check solution consistency wrt linkings. For each handle in
				 * the redex there shall be a compatible one in the agent where
				 * compatibility means that ports are matched with ports
				 * connected together and within the image of the redex into the
				 * agent and that inner names are connected to ports in the
					...
					for (PlaceEntity j : row.keySet()) {
						...
						if (solver.getVar(row.get(j)).getVal() == 1) {
							if (j.isNode()) {
								 ...
								 * param instantiation removes edges having
								...
								if (i.isNode()) {
									for (Node n : agent_ancestors
										...
										for (Port p : n.getPorts()) {
											// removes the handle of this port
											// since
				...
				for (Handle ha : handle_img.values()) {
					 ...
					 * expected, none of their point can't be in the context if
					 * ha has points in the params, then one of its images has
					 * to have at least an inner name.
					 */
					int f = 0;
				 ...
				 * bigaph mathing. However, there are many possible
				 * instantiations for its parameters due to the combinatorics
				 * introduced by inner names and redex handles not in
				 * handle_img.keyset(). In fact, if a redex handle isn't in
				 * handle_img then its only points are inner names and in the
				 * case of outernames they may have no points at all. These can
				 * be mapped more or less on every handle of the agent. inner
				 * names and outer names without points introduce some
				 * combinatorics in the way they can be matched. These
				 * assignments are computed as the solution of the following CSP
				 */
				...
				for (Handle h1 : redex_handles) {
					...
					if (handle_img.containsKey(h1)) {
						...
						if (aliased_inners.containsValue(h1)) {
							 ...
							 * edge, it can be assigned only to non_ctx_edges
							 * otherwise, to every agent handle
							 */
							...
							for (Handle h2 : pht_edges) {
								...
								lnk_model.addVariable(var);
								hr.put(h2, var);
								// enable this variable obly if the phantom edge
								// can
					...
					if (aliased_inners.containsValue(h1)) {
						for (Handle h2 : hr.keySet()) {
							// add vars for h1's inners (if any) and h2's ports
							// (if
							// any)
				...
				do {
					...
					// //////////////////////////////////////////////
					/*
					 * visits the redex but replicates its image in the agent
					 * (when possible)
					 */
